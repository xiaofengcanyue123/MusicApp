...
export default class RotateList extends Component {
    ...
    renderpage = (musicIndex) => {
        var { height, width } = Dimensions.get('window');
        let { musiclist } = this.props.player
        return (
            <View style={{
                alignItems: 'center',
                width: width,
    ...
    render() {
        var { height, width } = Dimensions.get('window');
        let { musiclist, length, musicIndex } = this.props.player
        let pages = []
        pages = Object.keys(musiclist)
        pages.unshift(length - 1 + '')
        pages.push('0')
        pages = musiclist.map((item, index) => {
            if (length >= 1) {
                if (index >= musicIndex && index <= musicIndex + 2) {
                    return this.renderpage(pages[index])
                }
            }
        })
        return (
